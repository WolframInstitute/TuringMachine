#![cfg(test)]
use ndtm_search::{
    dtm_output_table_steps_parallel,
    dtm_output_table_steps_parallel_f64,
};
...
fn test_steps_f64_consistency_small_inputs() {
    let s: u32 = 2;
    let k: u32 = 2;
    let max_steps: u64 = 128; // limit for performance
    let max_input: u32 = 64;  // keep values small enough for precise f64

    let base: u64 = (2 * s * k) as u64;
    let exp: u32 = (s * k) as u32;
    let rule_space_size: u64 = base.pow(exp); // total rules = 4096

    let structured = dtm_output_table_steps_parallel(s, k, max_steps, 0, rule_space_size - 1, 0, max_input);
    let flat = dtm_output_table_steps_parallel_f64(s, k, max_steps, 0, rule_space_size - 1, 0, max_input);
    ...
    assert_eq!(flat.len(), num_rules * num_inputs * 2, "Flat array length mismatch");
    ...
    for (r_idx, row) in structured.iter().enumerate() {
        ...
        for (i_idx, cell) in row.iter().enumerate() {
            let offset = (r_idx * num_inputs + i_idx) * 2;
            let step_f64 = flat[offset];
            let value_f64 = flat[offset + 1];
            match cell {
                Some((steps, value_big)) => {
                ...
                None => {
                    ...
                    assert_eq!(value_f64, 0.0, "Expected value=0.0 for non-halting at rule {} input {}", r_idx, i_idx);
                }
            }
        }
    }
