...
- Item 1 (major): closed-form initial condition and finish-time bounds. The tape of
  `Smith.conjecture0_finite` and `Smith.wolfram23_universal` is existential and sized
  ...
  `docs/PLAN.md` section 2 promises it; the M3 notes say it is undone; no declaration
  named `finishTime` exists); (ii) a System 4 exit-time bound from the explicit step
  counts of `Smith.dStep`, `Smith.popPhase`, `Smith.repS4_terminal`; (iii) a bound on
  the tag steps of {ref "tm-to-cts"}[the chapter on the machine reduction] in terms
  of `|c| + n` (the round lengths are explicit in `TagSystem.tm_step_tag`); (iv) a
  lemma that T7's cyclic tag run continues for any budget at or above the needed one,
  including after `tm` halts (`BiTM.step` is `none` in state 0 and the Cocke-Minsky
  ...
  `Smith.wolfram23_infinite` is met by a machine that only moves right over an
...
- Item 2: the infinite form T6 ({ref "infinite-form"}[the chapter on the infinite form]),
  done 2026-09-22, `Smith.wolfram23_infinite`. One right-infinite tape per machine and
...
- Item 3 (minor): binary machines. `TagSystem.WF` covers two-symbol machines; the
  reduction of k-symbol machines, or a bridge to Mathlib's `Turing.TM0`, is not
  formalized. Wording of `docs/PLAN.md` section 2 T7/T8 ("well-formed binary TM"):
  done.
...
- Item 6 (minor): `decodeCTS` soundness. Done 2026-09-22: `symbolDecode` requires the
  ...
  vectors in `Vectors/TMToCTSVectors.lean` (a short last block, a stray bit).
- Item 7 (minor): `ForwardSim` documentation. Done: the docstring of
  `Smith/Simulation.lean` says the content is in the relation being a decoder graph.
- Item 8: two unreachable branches of `BiTM.System4.step` differ from `system4.pl`
  (rule 5 with the star last, rule 4 at index 0). Done: comments in
  `BiTM/System4.lean`, `docs/REVIEW.md` softened to "faithful on all well-formed
  tapes".

# B. Tests

- Item 9: no vector runs `Smith.decodeTM` or `Smith.undbl`; on the D9 positive tape
  ...
  (`Vectors/TMToCTSVectors.lean`), `decodeTM` rejecting the D9 and D10 words
  (`Vectors/SmithVectors.lean` D10). A positive `decodeTM` vector on a rendered tape is
...
- Item 10: no vector runs a machine into the halt state. Done: `tmH` in
  `Vectors/TMToCTSVectors.lean`, through the tag system, the cyclic tag system and the
  decoder.
- Item 11: `Vectors/SmithVectors.lean` header and `docs/PLAN.md` lines 273-277 say
  `native_decide` is used only for D5; D7 used it seven times and
  `Vectors/TMToCTSVectors.lean` four times. Done: D7 and `Vectors/TMToCTSVectors.lean` now
...
Section and line numbers refer to `docs/PLAN.md` unless stated otherwise.
...
- Item 18: M8 row (line 210) and M8 notes (line 1094): `BiTM.Machine` does not exist;
  the type is `TM.Machine`. The statement also mentions `TagSystem.WF`,
  `TagSystem.ValidCfg`, `BiTM.IsValidWolfram23Cfg`, `TagSystem.canon`, `Smith.biSize`;
...
- Item 21: `Smith/Universality.lean` header lines 4-6 ("Every run ... from a finite
  initial tape"): loose; the docstring at lines 48-56 is correct. Header reworded
  (done); the name is kept, item 2 being done.
- Item 22: `Smith/Lookahead.lean` header: `exitRight` is not the observable exit of
  Conjecture 3 in the chain (two-cell rule at the right end); reword
  ({ref "machine-model"}[the chapter on the machine model]). Done.
- Item 23: `TagSystem/TagToCTS.lean` line 9: "The CTS has k appendants" should read
  "2k appendants: k production encodings followed by k empty appendants". Done.
- Item 24: `Smith/Conjecture3.lean` imports `Smith/LoopFree.lean` only for
  `lnSteps_add`; move the lemma to `Smith/Lookahead.lean` so that the import graph
  does not suggest T3 uses T5. Done.
- Item 25: `docs/REVIEW.md` line 161 (item 8 above) and the status notes: add one
  after T6. Done.
